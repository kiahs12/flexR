...
```
```{r, echo= FALSE}
filtered_data%>% #plot one, name later, normalized Fmax vs Angle
  ggplot(aes(x=angle,y=normF,col=activity))+geom_point()

dat_Fmax_norm_mean<-filtered_data%>%
...
shift.p_long %>%
  ggplot(aes(x=who, y=value, fill=Condition)) + geom_bar(stat="identity", position="dodge", width=0.5)+ scale_x_discrete(labels=NULL)+theme_classic()+coord_flip()+ylab("Theta max")+ggtitle("Figure 1: Theta max for each individual subject")
```
