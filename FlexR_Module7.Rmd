...
```{r, echo=FALSE}
f <- list.files("new_data2", pattern = ".csv", full.names=TRUE)
dat.l <- list()
...
for(i in f){
  ...
  dat.l[[i]]<- read_csv(i,show_col_types = FALSE) %>%
...
```
```{r, echo= FALSE}
dat_Fmax_norm%>% #plot one, name later, normalized Fmax vs Angle
...
dat_Fmax_norm_mean%>% #plot two, name later, mean normalized Fmax vs Angle
  ggplot(aes(x=angle,y=Fmax_norm_mean,col=activity))+geom_point()+labs(y="Mean Normalized Fmax", x="Angle")
```
```{r, echo= FALSE}
class(dat_Fmax_norm$angle)
poly.m2 <- lm(Fmax_norm~poly(as.numeric(angle),2),data=dat_Fmax_norm) 
poly.m3 <- lm(Fmax_norm~poly(as.numeric(angle),3),data=dat_Fmax_norm) 
poly.m4 <- lm(Fmax_norm~poly(as.numeric(angle),4),data=dat_Fmax_norm) 
AICc(poly.m2,poly.m3,poly.m4)
x.pred <- seq(45,157.5,length.out = 1000)
normF.pred <- predict(poly.m3,newdata = data.frame(angle=x.pred)) #predict the force using 1000 angles


qplot(ang,dat_Fmax_norm_mean)+geom_point(aes(x=x.pred,y=normF.pred),col="red")+geom_point(aes(x=x.pred[which.max(normF.pred)],y=normF.pred[which.max(normF.pred)]),size=5,col="blue")
```
